  ...
  function uploadFile($file, $name=NULL)
  {
    if(!empty($file)) {
      ...
      if($size > '2204800'){
        ...
        $destinationPath = public_path() . '/img/upload/' . date('m') . '/';
        if(is_null($name))
          $filename = date('s') . $file->getClientOriginalName();
        else
          $filename = $name;
        $file->move($destinationPath, $filename);
        return ('/img/upload/' . date('m') . '/' . $filename);
      }
    }
  }

  function uploadFiletToS3($file)
  {
    $destinationPath = 'weekender2014/upload/' . (date('ymdhis') . $file->getClientOriginalName());
    $s3 = App::make('aws')->get('s3');
    $s3->putObject(array(
      'Bucket'     => 'nh7',
      'Key'        => 'weekender2014/upload/' . (date('ymdhis') . $file->getClientOriginalName()),
      'SourceFile' => $file->getRealPath(),
      'ACL'    => 'public-read',
      ));
    return ('http://media.nh7.in/' . $destinationPath);
  }
